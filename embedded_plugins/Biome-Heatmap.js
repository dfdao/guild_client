...
spaceTypeColor[4] = "#ff0000" + spaceTypeOpacity; // SPAWN AREA!!!! (MODIFIED BY BUL)
...
function calcSpaceTypeColor(coords) {
    //! MODIFIED BY BUL
    const perlin = df.spaceTypePerlin(coords);
    if (13 < perlin && perlin <= 14) {
        return spaceTypeColor[4]
    }
    let spaceType = df.spaceTypeFromPerlin(perlin);
    //! MODIFIED BY BUL
